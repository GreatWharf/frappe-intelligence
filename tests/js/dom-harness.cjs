...
class Element {
  ...
  set textContent(value) { if (this.tagName === '#text') this.data = String(value); else { this.childNodes.forEach((node) => { node.parentNode = null; }); this.childNodes = []; const node = new Element('#text', this.ownerDocument); node.data = String(value); this.appendChild(node); } }
  get innerHTML() { return this.childNodes.map((node) => node.outerHTML).join(''); }
  set innerHTML(value) { this.childNodes.forEach((node) => { node.parentNode = null; }); this.childNodes = []; parse(String(value), this); }
...
class JSDOM {
  constructor(html) {
    ...
    const window = { document, URL, console, Event, KeyboardEvent: Event, navigator: { onLine: true }, Promise, Map, Set, Date, JSON, Object, Array, String, Number, Math,
