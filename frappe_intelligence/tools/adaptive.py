...
create_document and update_document write flat scalar fields of explicitly
write-scoped DocTypes. The never-allow set (hardcoded, settings-independent)
plus the is_submittable rejection keep ledger posting, workflow bypass and
submittable-document edits out of a generic field setter. Child tables are
read-only: no child-row payloads are accepted in this release.
"""
...
_CREATE_DOCUMENT = object_schema({"doctype": NAME, "fields": _FIELD_LIST}, ("doctype", "fields"))
...
def _writable(doctype, meta):
    return (
        not write_blocked(doctype)
        and not meta.is_submittable
        and doctype in policy_lines(settings().get("allowed_write_doctypes"))
    )
...
def _create_generic_plan(context, args):
    """Shared by preview_create_document and create_document; returns (doc, meta, missing)."""
    import frappe
    ...
    return doc, meta, missing


def preview_create_document(context, args):
    _, _, missing = _create_generic_plan(context, args)
    return {
...
def specs(context):
    return [
        ...
        ToolSpec(
            "create_document",
            "Create one draft document in a write-scoped DocType from flat scalar fields. No child rows, no submission, no ledger/workflow doctypes. Native validation and permissions apply.",
            _CREATE_DOCUMENT,
            create_document,
        ...
        ToolSpec(
            "update_document",
            "Update flat scalar fields of one existing write-scoped draft document; supply its exact modified timestamp. No child rows, no submission, no ledger/workflow doctypes.",
            _UPDATE_DOCUMENT,
            update_document,
